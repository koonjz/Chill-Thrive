...
const dateInput = document.getElementById("date"); const timeSelect = document.getElementById("time"); if (dateInput && timeSelect) {const today = new Date().toISOString().split("T")[0]; dateInput.setAttribute("min", today); const defaultSlots = [ "07:00 – 10:00", "10:00 – 13:00", "15:00 – 18:00", "18:00 – 21:00" ]; const slotTimings = { "07:00 – 10:00": 7, "10:00 – 13:00": 10, "15:00 – 18:00": 15, "18:00 – 21:00": 18 }; dateInput.addEventListener("change", () => { loadSlots(dateInput.value); }); function loadSlots(selectedDate) { timeSelect.innerHTML = <option value="">Select Time</option>; const now = new Date(); const isToday = selectedDate === now.toISOString().split("T")[0]; let availableCount = 0; let validSlotCount = 0; defaultSlots.forEach(slot => { const slotHour = slotTimings[slot]; // ❌ Skip past slots if today if (isToday && now.getHours() >= slotHour) return; validSlotCount++; const docId = ${selectedDate}_${slot}; db.collection("timeSlots").doc(docId).get().then(doc => { let capacity = 5; let booked = 0; if (doc.exists) { capacity = doc.data().capacity; booked = doc.data().booked; } if (booked < capacity) { availableCount++; const option = document.createElement("option"); option.value = slot; option.textContent = ${slot} (${capacity - booked} slots left); timeSelect.appendChild(option); }if (isToday && validSlotCount === 0) { dateInput.setCustomValidity("All time slots for today have already passed."); dateInput.reportValidity(); }else if (availableCount === 0 && slot === defaultSlots[defaultSlots.length - 1]) { dateInput.setCustomValidity("No slots available on this date."); dateInput.reportValidity(); }else { dateInput.setCustomValidity(""); } }); }); } }
